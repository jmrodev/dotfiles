...
show_help() {
    ...
    echo -e "  $0 --path ~/.config/i3/config.custom  Usa un archivo de configuración alternativo"
    exit 0
}
...
process_config_file() {
    ...
    while IFS= read -r line; do
        ...
        if [[ "$line" =~ ^\s*(#\s*)?(bindsym|bindcode) ]]; then
            ...
            # Reemplazar variables en la combinación de teclas
            ...
            if [[ "$command" =~ workspace ]]; then
                ...
                category="Ventana"
            elif [[ "$command" =~ i3-nagbar|i3bar|i3status|i3lock|i3-msg ]]; then
                category="i3"
            ...
            # Formatear estado (activo/comentado)
            status="[ACTIVO]"
            if [ "$is_commented" = true ]; then
                status="[COMENTADO]"
            fi
            
            # Formatear tipo de atajo
            type_indicator="[SYM]"
...
# Procesar el archivo de configuración principal
process_config_file "$CONFIG_PATH"

# Función para mostrar el menú de Rofi
show_rofi_menu() {
    # Extraer categorías y modos únicos
    categories=$(cat "$TEMP_FILE" | cut -d'|' -f1 | sort | uniq)
    modes=$(cat "$TEMP_FILE" | cut -d'|' -f2 | sort | uniq)
    
    # Preparar opciones de menú
    ...
    selection=$(echo -e "$menu_options" | rofi -dmenu -i -p "Atajos i3" \
    ...
    case "$selection" in
        ...
        "MOSTRAR ATAJOS COMENTADOS")
            SHOW_COMMENTED=true
            # Reprocesar el archivo de configuración
            > "$TEMP_FILE"
            process_config_file "$CONFIG_PATH"
            show_bindings
            ;;
        "OCULTAR ATAJOS COMENTADOS")
            ...
            ;;
        *)
            # Si no se seleccionó nada, mostrar los atajos
            show_bindings
            ;;
    esac
}

# Función para mostrar los atajos filtrados
show_bindings() {
    # Aplicar filtros
    filtered_content=""
    ...
    # Mostrar atajos en Rofi
    selected_binding=$(echo -e "$filtered_content" | rofi -dmenu -i -p "Atajos i3" \
    ...
    # Manejar selección
    exit_code=$?
    if [ $exit_code -eq 10 ]; then  # F1 presionado
        show_rofi_menu
    elif [ $exit_code -eq 0 ]; then  # Selección normal
        # Aquí podrías hacer algo con la selección, como copiar al portapapeles
        # Por ahora, simplemente volvemos a mostrar los atajos
        show_bindings
    fi
}

# Iniciar mostrando los atajos
show_bindings
